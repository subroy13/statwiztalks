...
\graphicspath{{../../figures/wiser_llm/}}
\usepackage{booktabs}
...
\title[WISER]{\LARGE{Localized Detection of Authenticity of Mixed Human-LLM Generated Texts}}
\author[S.~Roy]{Subhrajyoty Roy} 
\institute[SLU]{Stat/Data seminar \\St. Louis University} 
\date{Feb 4, 2026} 
\titlegraphic{\includegraphics[width=7cm]{logo.jpg}} 
...
\begin{frame}{Introduction}
    \begin{itemize}
        \item \alert{Postdoctoral Researcher} in the Department of Statistics and Data Science of Washington University in St. Louis.
        \item Ph.D. (Statistics) from \alert{Indian Statistical Institute, Kolkata, India}.
        \item \textbf{Key Research Areas:}
        \begin{itemize}
            \item Robust Statistics.
            \item Dependent Data Analysis (Spatial Statistics and Spatiotemporal modelling).
            \item Statistical methods for Deep Learning.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Exponential Growth of LLM}
    \includegraphics[width=0.95\linewidth]{llm-expo-graph.png}
...
\begin{frame}{Rise of LLM Generated Content}
    \includegraphics[width=0.8\linewidth]{epium-content-generation.png}
    \includegraphics[width=0.6\linewidth]{opeds_genai_declaration.png}
    \includegraphics[width=0.38\linewidth]{nature article.png}
\end{frame}
...
\begin{frame}{Emerging Issues with Academic Integrity}
    \includegraphics[width=\linewidth]{emerging-academic-integrity.png}
\end{frame}

\begin{frame}{More Upcoming Threats!}
    \begin{enumerate}
        \item In order to train the next generation of models, we need new datasets.
        \item Datasets are curated from the internet, which already contains AI-generated texts.
        \pause
        \item Can create a vicious loop, decreasing fairness, increasing ``not-so-good'' qualities.
    \end{enumerate}
...
\begin{frame}
    ...
    \begin{center}
    \includegraphics[width=0.5\linewidth]{hopeless-meme.jpg}        
    \end{center}
...
\begin{frame}{The growth of ``explore''}
    ...
    \begin{figure}[h]
    \includegraphics[width = 0.8\linewidth]{explore word frequency.png}
...
\begin{frame}{The growth of ``delve into''}
    % https://arxiv.org/pdf/2504.08755
    \begin{figure}[h]
    \includegraphics[width = 0.8\linewidth]{delve-into-graph.png}
...
\begin{frame}{The three detection frameworks}
    \begin{center}
        \includegraphics[width = \textwidth]{detection-framework1.png}
        \includegraphics[width = 0.5\textwidth]{detection-framework2.png}        
    \end{center}
\end{frame}
...
\begin{frame}{Red Green Watermark (Contd.)}
    \includegraphics[width = \linewidth]{red-green-watermark.png}
...
\begin{frame}{Example}
    \begin{figure}[h]
        \centering
        \includegraphics[width=\linewidth]{text_example.jpeg}
        \includegraphics[width=\linewidth]{wm_diagram_pivot.png}
...
\begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
        \includegraphics[width = \linewidth]{wiser-stage1.png}
    \end{column}
\end{columns}
...
\begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
        \includegraphics[width = \linewidth]{wiser-stage2.png}
    \end{column}
\end{columns}
\end{frame}
...
\includegraphics[width = \linewidth]{wiser-stage-searchregion.png}
...
\begin{frame}{Assumptions (Contd.)}
    ...
    \begin{center}
        \includegraphics[width=\linewidth]{chatgpt-novar.png}
    \end{center}
...
\begin{frame}{Empirical Validation}
    \begin{enumerate}
        \item Pick open source models available in Huggingface: \texttt{facebook/opt-125m}, \texttt{Gemma-3-270m}, ..., \texttt{Llama 3.8B}.
        \item Pick a hash function to generate $\xi_t$s. 
        \item Randomly chosen $B = 500$ prompts from Google news dataset. 
        \item LLMs are asked to perform the completion of $n$ tokens based on $50$ input tokens. Some patches of $n$ tokens are generated by the watermarking scheme instead of sampling from $P_t$.
    \end{enumerate}
\end{frame}

\begin{frame}{Histogram of detected endpoints (one scenario)}
    \includegraphics[width = \textwidth]{wiser-output.png}
\end{frame}


\begin{frame}{Benchmark Results (Gumbel)}
    \begin{center}
        \includegraphics[width=\linewidth]{gumbel-result.png}
    \end{center}
\end{frame}

\begin{frame}{Benchmark Results (Red-Green)}
    \begin{center}
        \includegraphics[width=\linewidth]{redgreen-result.png}
    \end{center}    
\end{frame}

\begin{frame}{Time Complexity}
    \begin{center}
        \includegraphics[width=\linewidth]{time-complexity.png}\\
        Time complexity of \texttt{WISER} is $O(K n)$.
    \end{center}
\end{frame}

\begin{frame}{Sample size requirement}
    \begin{center}
        \includegraphics[width = \linewidth]{simulation2A_delta.png}
        
    $\delta$ is the amount of bias (in logit scale) for the Red-Green watermark.
    \end{center}
\end{frame}
...
\begin{frame}{Effect of Block Size}
    \begin{center}
        \includegraphics[width=\linewidth]{ablation_block_size.png}\\
...
%         \item \texttt{WISER} can be used to efficiently detect watermarked regions in mixed-source texts. It is fast, theoretically valid, and works across multiple models and multiple watermarking schemes.
%         \item Code available at \url{https://www.github.com/subroy13/wiser}
%     \end{itemize}
% \end{frame}
...
\begin{frame}{There are still things to do!}
    ...
    \begin{center}
        \includegraphics[width=0.8\textwidth]{emoji-attack.png}\\
...
\begin{frame}{}
    \begin{center}
        {
\small \textbf{Code:} \url{https://github.com/subroy13/wiser}\\
\small \textbf{Paper:} WISER: Segmenting watermarked region - an epidemic change-point perspective (\url{https://arxiv.org/pdf/2509.21160})\\
\small \textbf{Email:} roy.s@wustl.edu
}
    \vspace*{1in}\\
    {
